...
PImage start, help; //background image
...
ArrayList<Node> path = new ArrayList<Node>(); //keeps track of the path Pac-Xon is drawing with Nodes containing x and y
...
//------------------methods that create screens---------------

void createTitle() {
   rect((width-530)/2, height/2 , 550, 100, 6, 6, 6, 6);
   fill(255);
   textSize(100);
   textAlign(CENTER, CENTER);
   fill(0);
   text("CHECKERS", (width+10)/2, (height +50)/2);
}

void createStart() {
  rect(width/2, 80, 170, 70, 6, 6, 6, 6);
  fill(0);
  textSize(50);
